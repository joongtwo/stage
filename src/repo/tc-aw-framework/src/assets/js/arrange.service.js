...
export let columnVisibilityChanged = function( data ) {
    ...
    for( i = 0; i < data.arrangeData.columnDefs.length; ++i ) {
        ...
        if( data.arrangeData.columnDefs[ i ].propertyName === 'object_name' && !data.arrangeData.columnDefs[ i ].dbValue ) {
            data.arrangeData.columnDefs[ i ].dbValue = true;
        }

        if( data.arrangeData.columnDefs[ i ].dbValue === true ) {
            data.arrangeData.isColumnsSelected = true;
        }
    }

    for( i = 0; i < data.arrangeData.columnDefs.length; ++i ) {
        if( !data.arrangeData.columnDefs[ i ].dbValue ) {
...
export let actionFilterList = function( data ) {
    ...
    if( data.arrangeData.columnDefs === null ) {
        ...
        let columnOrder = 100;
        for( var i = 0; i < arrangeClientScopeUI.columns.length; ++i ) {
            var column = arrangeClientScopeUI.columns[ i ];
...
export let markDirty = function( data ) {
    ...
    if( data.arrangeData.orgColumnDefs.length !== data.arrangeData.columnDefs.length ) {
        ...
        for( var i = 0; i < data.arrangeData.orgColumnDefs.length; ++i ) {
