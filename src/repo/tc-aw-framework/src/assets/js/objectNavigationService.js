...
export const processOutput = ( data, dataCtxNode, objNavState = {} ) => {
    ...
    let { totalFound, searchFilterCategories, searchFilterMap6, objectsGroupedByProperty, columnConfig, propDescriptors } = data;
    ...
    newObjNavState.searchResponseInfo = {
        ...
        searchFilterMap: searchFilterMap6,
        objectsGroupedByProperty: objectsGroupedByProperty,
        columnConfig: columnConfig,
        propDescriptors: propDescriptors
    };
    if( dataCtxNode && dataCtxNode.subPanelContext && dataCtxNode.subPanelContext.selectionModel.getSelection() ) {
        const selectionsInSM =  dataCtxNode.subPanelContext.selectionModel.getSelection();
        if ( selectionsInSM.length === 1 && data.cursor.startIndex === 0 ) { // requried only for first page
            const matched = data.searchResults.filter( obj => {
                return selectionsInSM[0] === obj.uid;
            } );
            if ( matched && matched.length === 0 ) {
                dataCtxNode.subPanelContext.selectionData.update( {
                    selected: [ _.get( objNavState, 'defaultBaseSelection' ) ],
                    pselected: _.get( objNavState, 'defaultBaseSelection' ),
                    source: 'base'
                } );
            }
        }
...
export const getSubPanelContextData = ( provider, baseSelection, selectionModel ) => {
