...
var ViewerContextData = function( viewerCtxNamespace, viewerType, viewerView, is2D, viewerAtomicData ) {
    ...
    self.MEMORY_THRESHOLD_FOR_ACE_VIEWER = 3600; //~80% of total available memory 4398 MB
    ...
    var errorHandler = {
        error: function( err ) {
            ...
            } else if( err.name === 'BadTopomeshMeasurementError' || err.name === 'BadTopomeshSectionError' ) {
    ...
    self.viewerMemoryWarning = function() {
        if( !_memoryThresholdWarningShown ) {
            messagingService.showWarning( self.getThreeDViewerMsg( 'memoryConsumptionWarning' ) );
