...
export let getCreateInput = function( data, extensionVMProps, creationType, editHandlerIn ) {
    ...
    var workflowData = {};
    let processTemplates = _.get( createInputMap, 'revision.propertyNameValues.awp0ProcessTemplates' );
    ...
    if( response.output ) {
        for( var index in response.output ) {
            if( response.output[ index ].objects ) {
                ...
                if( newObject.modelType.typeHierarchyArray.indexOf( 'Item' ) > -1 && validTypes.indexOf( newObject.type ) === -1 ) {
 ...
 */
export let initDSCreateParams = function( data, datasetInfo, addPanelState, isCustomDatasetAction ) {
    ...
    const tabChangeCallback = ( pageId, tabTitle ) => {
        ...
        if( addPanelState ) {
            ...
            if( selectedTab ) {
                ...
                if( selectedTab.pageId === 'new' && newAddPanelState.sourceObjects ) {
                    newAddPanelState.sourceObjects = [];
                } else if( selectedTab.pageId === 'palette' || selectedTab.pageId === 'search' &&
