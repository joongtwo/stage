...
const defaultXRT = `<?xml version="1.0" encoding="UTF-8"?>
<!-- Empty Rendering -->
<rendering>
</rendering>`;
...
export const awXrtEditorOnMount = ( data ) => {
    ...
    const inputData = {
        input: [ {
            boTypeName: 'WorkspaceObject',
            exclusionBOTypeNames: []
        } ]
    };
    let objectTypeList = [];
    soaService.post( 'Core-2010-04-DataManagement', 'findDisplayableSubBusinessObjectsWithDisplayNames', inputData )
        .then( function( response ) {
            if( response ) {
                for( const output of response.output ) {
                    for( const displayableBOTypeName of output.displayableBOTypeNames ) {
                        objectTypeList.push( {
                            propDisplayValue: displayableBOTypeName.boDisplayName,
                            propInternalValue: displayableBOTypeName.boName,
    ...
    data.dispatch( { path: 'data.objectTypeList', value: objectTypeList } );
    ...
    configurationService.getCfg( 'states' ).then( function( states ) {
        ...
        data.dispatch( { path: 'data.content', value: defaultXRT } );
    } );
};
...
export const awXrtEditorRenderFunction = ( props ) => {
    ...
    return data.editor && data.content && <AwPanelBody>
        ...
        <div className='sw-row aw-xrteditor-header'>
            ...
            <AwListbox className='aw-xrteditor-listbox' {...fields.objectTypeListBox} list={data.objectTypeList}></AwListbox>
    ...
    xrtEditorUtils.loadXRT( type, stylesheetType, preferenceLocation, client, location, sublocation ).then(
        function( dsInfo ) {
            ...
            if( data.content !== dsInfo.xrt ) {
                data.dispatch( { path: 'data.content', value: dsInfo.xrt !== '' ?  dsInfo.xrt : defaultXRT } );
            }
