...
export const awAddRenderFunction = ( {
    ...
    let optionalTabs = viewModel.data.optionalTabs;
        ...
        compute: ( renderContext, visibleTabs, defaultTab, optionalTabs ) => {
            ...
            for( let optionalTabData of optionalTabsData ) {
                if( optionalTabData.visibleWhen && visibleAddPanelTabKeys.indexOf( optionalTabData.key ) > -1 ) {
                    if( defaultTabKey === optionalTabData.name ) {
                        addPanelTabModels.push( _buildTabModel( optionalTabData.name, optionalTabData.name, optionalTabData.view, optionalTabData.name, optionalTabData.priority, true ) );
                    } else {
                        addPanelTabModels.push( _buildTabModel( optionalTabData.name, optionalTabData.name, optionalTabData.view, optionalTabData.name, optionalTabData.priority ) );
                    }
                }
            }
