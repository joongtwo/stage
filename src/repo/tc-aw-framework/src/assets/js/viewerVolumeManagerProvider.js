...
class ViewerVolumeManager {
     ...
     */
    comparetargetsWithSelections( subPanelContext, volumeState ) {
        let targetList = [];
        let selectionList = [];
        let i = 0;
    ...
    executeVolumeSearch( cVs ) {
        ...
        return JSCom.jQueryPLMVis.when( valSetPromises ).done( () => {
    ...
    getCornerValuesFromOccListInCtx( volumeState ) {
        ...
        this.viewerView.volumeMgr.updateVolumeFromOccurrenceList( occObjList ).then( function() {
            ...
            return JSCom.jQueryPLMVis.when( valGetPromises ).done( function( minVals, maxVals ) {
    ...
    drawVolumeBox( cornerVals, volumeState ) {
        ...
        JSCom.jQueryPLMVis.when( valSetPromises ).done( ()=> {
