...
export let updatebaseline = function( input ) {
    return input[ 0 ] === '2';
};
/**
 ...
 */
export let getReviseInputs = function( editHandler ) {
    let reviseInputs = {};
    if( editHandler ) {
        let dataSource = editHandler.getDataSource();
        if( dataSource ) {
...
export let getBaselineTemplateList = function( strings ) {
    var listModels = [];
...
/*
* Update precise baseline checkbox value based on preference value
*@param {String} preference - Value of 'Baseline_precise_bvr' preference
*@param {Object} baselinePrecise - check box value to be updated
 */
export let updateIsPreciseFlag = function( preference, baselinePrecise ) {
    if( preference === '2' ) {
        baselinePrecise.dbValue = true;
    }
    return baselinePrecise;
};
