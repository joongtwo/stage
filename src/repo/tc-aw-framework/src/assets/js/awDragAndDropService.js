...
const executeFileDrop = ( targetVMO, callbackAPIs, pasteRelation2SourceObjectsMap ) => {
    _scheduleSelectTarget( targetVMO, callbackAPIs );

    /**
     * Paste 'sources' to 'target' for each unique 'relation' type.
     */
    var pasteInput = createPasteInput( pasteRelation2SourceObjectsMap, targetVMO );
...
var _pasteSourceFiles = function( targetVMO, sourceFiles, dragAndDropParams, validSourceTypes ) {
    ...
    if( cmm.isInstanceOf( 'Awp0XRTObjectSetRow', targetVMO.modelType ) ) {
        adapterSvc.getAdaptedObjects( [ targetVMO ] ).then( function( adaptedObjs ) {
            executeFileDrop( adaptedObjs[ 0 ], dragAndDropParams.callbackAPIs, pasteRelation2SourceObjectsMap );
        } );
    } else {
        executeFileDrop( targetVMO, dragAndDropParams.callbackAPIs, pasteRelation2SourceObjectsMap );
    }
};
