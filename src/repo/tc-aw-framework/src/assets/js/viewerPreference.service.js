 ...
 * @param {String} renderSource render source value
 */
export let setRenderSource = function( renderSource, viewerContextData ) {
    updatePreferenceValue( VIEWER_RENDER_OPTION, [ renderSource ], true, viewerContextData );
};

/**
 * get rendering source
 * @returns {String} returns render source preference value
 */
export let getRenderSource = function( viewerContextData ) {
    return getPreferenceValue( VIEWER_RENDER_OPTION, viewerContextData );
};
...
/**
 * get csr loading strategy Preference for viewer
 * @param {String} prefValue preference value
 * @returns {Number} csr loading strategy
 */
let getCSRLoadingStrategy = function( prefValue ) {
    if( prefValue.toUpperCase() === 'NONE' ) {
        return CSRLoadingStrategy.None;
    } else if( prefValue.toUpperCase() === 'USESMARTLOADING' ) {
        return CSRLoadingStrategy.useSmartLoading;
    }
    let value = parseInt( prefValue );
    if( !isNaN( value ) && value > 1 && value < 6 ) {
        return value;
    }
    return CSRLoadingStrategy.useSmartLoading;//should always take default pref value if garbage value is passed
};
...
let getMemoryThreshold = function( viewerContextData ) {
    if( viewerContextData ) {
        if( appCtxService.getCtx( 'splitView.mode' ) ) {
            return viewerContextData.MEMORY_THRESHOLD_FOR_ACE_VIEWER / 2;
        }
        return viewerContextData.MEMORY_THRESHOLD_FOR_ACE_VIEWER;
    }
};
...
var initViewerPreferences = function( isShowAll, applyBookmarkWhileOpeningModel, disableBookMark, showSuppressed, selectionLimit, viewerContextData ) {
    ...
    var allViewerPrefs = [ NAVIGATION_MODE, NAVIGATION_3D_MODE, SHADING, MATERIAL, TRIHEDRON,
        ...
        VIEWER_SELECTION_DISPLAY, VIEWER_ZOOM_IN, ALL_ON, VIEWER_RENDER_OPTION, DISPLAY_UNIT, VIEWER_EXPOSED_BETA,
        VIS_COLOR_SCHEME, VIS_BROWSER_CACHING, VIEWER_APPLY_DEFAULT_MODEL_VIEW, VIS_CSR_LOADING_STRATEGY
    ];
    ...
    var allViewerPrefsDefaultVals = [ 'ROTATE', 'EXAMINE', 'false', '4',
        ...
        'true', 'false', 'false',
        '-z', '+x', '+y', 'true',
        'false', 'true', 'Push', 'true', 'SSR', '3', '', 'siemens', 'true', 'false', 'useSmartLoading'
    ];
    ...
    viewerPrefPromise.then( function( viewerPrefValuesMap ) {
        ...
        if( _openModelPreferences.performance.bboxSS ) {
            _openModelPreferences.performance.bboxSS.policy =  getCSRLoadingStrategy( viewerPrefValuesMap[ VIS_CSR_LOADING_STRATEGY ][0] );
        }
...
var updateViewerPreferences = function( viewerContextData ) {
    if( _openModelPreferences && viewerContextData ) {
        ...
        updatedPreferences.AWV0ViewerRenderOption = _openModelPreferences.renderSource;
