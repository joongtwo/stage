 ...
 */
export let populateImportPALData = function( data ) {
    var userOption = '';
    var context = null;

    if( data.overwriteDuplicatePAL.dbValue ) {
...
export let getExportFileName = function( fileName ) {
    ...
    var fileExtension = _getFileExtension( fileName );
    if ( !fileExtension ) {
        fileName += '.xml';
    }
    return fileName;
...
export let viewExportedLogFile = function() {
    setTimeout( function() {
        eventBus.publish( 'exportPAL.openLogFile' );
    }, 1000 );
};
