...
( function() {
    ...
    CKEDITOR.plugins.add( 'epautocomplete', {
    ...
    View.prototype = {
        ...
        setPosition: function( rect ) {
            ...
            editorViewportRect = $( ".aw-epInstructionsEditor-wiEditorContainer" )[ 0 ].getBoundingClientRect();
            ...
            // set overflow to hidden to position panel in available bounds.
            document.body.style.overflow = 'hidden';

            //Set panelWidth to offsetWidth to position panel in available bounds.
            var panelWidth = document.getElementsByClassName( 'cke_autocomplete_panel' )[ 0 ].offsetWidth;

            //Added Check to make sure panel is fully visible and remains in available bounds.
            if( panelWidth === 0 ) {
                panelWidth = document.getElementsByClassName( 'cke_autocomplete_panel' )[ 1 ].offsetWidth;
            }

            var availableBodyWidth = this.document.getBody().$.offsetWidth - rect.left;

            // adjust height to 90% of available space, after taking toolbar's height in consideration.
            //showing minimum of 4 items in the list
            var maxPanelHeight = ( spaceAbove >= spaceBelow ? spaceAbove : spaceBelow ) * 0.90 > 170 ? ( spaceAbove >= spaceBelow ? spaceAbove : spaceBelow ) * 0.90 : 170;

            if( panelWidth > availableBodyWidth ) {
                this.element.setStyles( {
                    right: availableBodyWidth + 'px',
                    top: top + 'px',
                    left: 'auto',
