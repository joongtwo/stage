...
/**
 * @module js/epActivitiesSaveHandlerService
 */
...
function saveEdits( dataSource ) {
    ...
    modifiedProperties.forEach( ( modifiedProperty ) => {
        ...
        modifiedProperty.viewModelProps.forEach( ( viewModelProp ) => {
            let dbValues = viewModelProp.dbValues;
            let value = viewModelProp.dbValue;
            dbValues[0] = value.toString();
            saveInputWriter.addModifiedProperty( modifiedProperty.viewModelObject.uid, viewModelProp.propertyName, dbValues );
        } );
    } );
