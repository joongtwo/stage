...
let deselectUnconfiguredElementsFromTree = function( deletedElements ) {
    eventBus.publish( 'aceElementsDeSelectedEvent', {
        elementsToDeselect: deletedElements,
...
export let afterSplitEleUpdatedProcessSplitResponse = function( response ) {
    ...
    deselectUnconfiguredElementsFromTree( response.ServiceData.deleted );
...
saveHandler.saveEdits = function( datasource, inputs ) {
    ...
    if( appCtxSvc.ctx.userSession.props.cm0GlobalChangeContext.dbValue ) {
