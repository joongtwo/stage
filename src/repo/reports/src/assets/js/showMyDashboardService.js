...
let getReportsListFromPreference = ( preferenceName )=> {
    ...
    if( preferenceName && appCtxService.ctx.preferences[ preferenceName ] ) {
...
export let processSearchResults = ( response, searchString ) => {
    var searchResults = [];
    response.reportdefinitions.forEach( reportdef => {
        var searchResult = {
            propDisplayValue: reportdef.reportdefinition.props.rd_name.dbValues[0],
            propDisplayDescription: reportdef.reportdefinition.props.rd_description.dbValues[0],
            propInternalValue: reportdef.reportdefinition.props.rd_id.dbValues[0],
            reportType: reportdef.reportdefinition.props.rd_type.dbValues[0],
            reportdefinition: reportdef.reportdefinition
        };
...
export let getUIDForSelectObject = ( ctx ) => {
    if( ctx?.ReportsContext?.selected?.props?.rd_type?.dbValues[ 0 ] === '1' ) {
        var obj = reportsCommSrvc.getUnderlyingObject( ctx.selected );
        return obj.uid;
    }
    return undefined;
};
