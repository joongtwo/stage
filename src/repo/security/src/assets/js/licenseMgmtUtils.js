...
var eachObjectInMultiSelectInput = function( selectedObject, data, isACE, attachFlag ) {
    var object = exports.isSupportedTCVersion() ? selectedObject :
...
export let attachDetachInput = function( data, attachFlag ) {
    ...
    if( size > 0 ) {
        _.forEach( selectedObjects, function( object ) {
            licenseObject = eachObjectInMultiSelectInput( object, data, isACE, attachFlag, size );
            attachOrDetachInput.push( licenseObject );
        } );
    } else {
        if( isACE ) {
            licenseObject = rootObjectACEInput( selectedObjects, data, isACE, attachFlag );
            attachOrDetachInput.push( licenseObject );
        }
    }
