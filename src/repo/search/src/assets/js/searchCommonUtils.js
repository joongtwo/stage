    ...
    for( const [ key, value ] of Object.entries( activeFilterMap ) ) {
        for( let index = 0; index < value.length; index++ ) {
            if( activeFiltersMap[ key ] && activeFiltersMap[ key ].length > 0 ) {
                ...
                existingValues.push( value[ index ].stringValue );
                activeFiltersMap[ key ] = existingValues;
            } else {
                activeFiltersMap[ key ] = [ value[ index ].stringValue ];
            }
        }
    }
...
export const updateSearchState = ( searchString, subPanelContext ) => {
    ...
    if( newSearchState.criteria.searchString && hasSearchStringChanged === false ) {
        newSearchState.criteria.searchString = toggleSearchString( newSearchState.criteria.searchString );
    }
    newSearchState.criteriaJSONString = JSON.stringify( newSearchState.criteria );
...
export const updateSearchStateAfterFilterAction = ( searchState ) => {
    const newSearchState = { ...searchState.value };
