...
export let doAddToChangedPreferences = function( viewModelPrefs ) {
    ...
    _.map( viewModelPrefs, function( item ) {
        if( item.value !== item.dbValue && item.valueUpdated === true && item.propertyName !== 'AWC_Limited_Filter_Categories_Expanded' ) {
...
export let loadViewModel = function( data ) {
    ...
    let expandedPref = prefs.AWC_Limited_Filter_Categories_Expanded;
    ...
    if( data.AWC_Limited_Filter_Categories_Enabled.dbValue === true || data.AWC_Limited_Filter_Categories_Enabled.dbValue === 'true' ) {
 ...
 */
export let getFiltersExpanded = function( prop, isDefault, data ) {
    let filterListStringArray = isDefault ? data.defaultExpandedFilterList : data.custom;
    ...
    return {
        filterExpandedListInUse: filterExpandedListInUse,
        newprop: newprop
    };
};
...
export let saveSettings = ( data, searchState ) => {
    ...
    _.forEach( keys, function( propertyName ) {
        ...
        if( eachProp.isArray && eachProp.dbValue && eachProp.dbValue.length > 1 ) {
            ...
            let tempVal = [ eachProp.dbValue || eachProp.dbValue === false || eachProp.dbValue === 0 ? eachProp.dbValue.toString() : '' ];
...
export let loadData_FilterExpanded = function( prop, data ) {
    let output = searchSettingsService.getFiltersExpanded( prop, true, data );

    let newProp = output.newprop;
    var lovEntries = [];
    _.forEach( output.filterExpandedListInUse, function( entry ) {
        let lovEntry = {
