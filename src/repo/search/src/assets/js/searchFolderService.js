...
/**
  ...
  * @param {Object}uidFolder - uid of Active Folder
  */
export let updateState = function( searchFolderSelected, searchFolder, searchFolderUpdater ) {
    const updateAtomicData = searchFolderUpdater.searchFolder;
    let newSearchState = {};
    ...
    if( searchFolderSelected.awp0SearchType === UNKNOWN ) {
        ...
        newSearchState.activeFilterMap = newSearchState.searchFolderFilterMap;
        let activeFiltersInfo = searchCommonUtils.createActiveFiltersFromActiveFilterMap( newSearchState.activeFilterMap );
    ...
    updateAtomicData( { ...searchFolder, ...newSearchState } );
};
...
export let updateNonEditState = ( searchFolder, newSearchFolder, translatedSearchString ) => {
    ...
    if( newSearchFolder.awp0SearchType === UNKNOWN ) {
        ...
        let activeFiltersInfo = searchCommonUtils.createActiveFiltersFromActiveFilterMap( newSearchFolder.activeFilterMap );
...
export let updateCriteriaAndFilters = function( data, searchFolderCtx ) {
    ...
    if( searchFolderCtx.isRuleDirty ) {
        ...
        if( searchFolderCtx.awp0SearchType === ADVANCED_PROVIDER && searchFolderCtx.props ) {
            ...
            data.searchFolderFilters.uiValue = searchFolderCtx.searchFilters ? _.join( searchFolderCtx.searchFilters, '\n' ) : '';
        }
    }
...
/**
  ...
  * @param {Object}searchFolderCriteria - searchFolderCriteria
  */
export let setSearchFiltersAfterSave = function( prop, value, searchFilters, index, searchFolderCriteria ) {
    delete value.colorValue;
    delete value.count;
    delete value.selected;
    if( value.searchFilterType === 'StringFilter' ) {
        searchFolderService.setSearchFiltersAfterSave_procStringFilter( prop, value, searchFilters );
    } else if( value.searchFilterType === 'DateFilter' ) {
        searchFolderService.setSearchFiltersAfterSave_procDateFilter( prop, value, searchFilters );
    } else if( value.searchFilterType === 'NumericFilter' ) {
        searchFolderService.setSearchFiltersAfterSave_procNumericFilter( prop, value, searchFilters );
    } else if( value.searchFilterType === 'RadioFilter' ) {
        searchFolderService.setSearchFiltersAfterSave_processRadioFilter( prop, value, searchFilters );
    }
...
export let getFulltextReportDefinitionCriteriaForSave = function( searchFolderCtx ) {
    ...
    let index = -1;
    for( var prop in searchFolderCtx.activeFilterMap ) {
        if( searchFolderCtx.activeFilterMap.hasOwnProperty( prop ) ) {
            _.forEach( searchFolderCtx.activeFilterMap[ prop ], function( value, key ) {
                ++index;
                //need to get stringDisplayValue from searchFolderFilterMap
                _.forEach( searchFolderCtx.searchFilterMap[ prop ], function( value2, key2 ) {
                    if( value.stringValue === value2.stringValue ) {
                        searchFolderService.setSearchFiltersAfterSave( prop, value2, searchFilters, index, criteria );
                    }
                } );
            } );
        }
...
export let getAdvancedReportDefinitionCriteriaForSaveInt = function( criteria, searchFilters, searchFolderCriteria, data ) {
    let index = -1;
    for( var prop in data.searchCriteriaMap ) {
...
export const setAdvancedSearchCriteriaMap = ( searchFolderCtx, searchCriteriaMap ) => {
    ...
    for( var prop in searchFolderCtx.savedQueryAttributes ) {
