...
export default class GlobalSearchService extends AwBaseService {
    ...
    populatePrefilters2( data ) {
        ...
        if( this._dataSourcesReceivedFromServer.length > 0  ) {
            ...
            newData.selectPrefilter3.uiValue = prefilter3Val.toString();
            newData.selectPrefilter3.uiValues = prefilter3Val;
    ...
    /**
      ...
      */
    getDataSourceList() {
        preferenceSvc.getStringValues( 'XST_cross_domain_sources' ).then( ( prefValue ) => {
            if( !prefValue ) {
                this._dataSourcesReceivedFromServer = [];
            }else{
    ...
    getSelectedDataSources( data ) {
        ...
        preferenceSvc.getStringValues( name ).then( ( prefValue ) => {
            ...
            viewModelPrefs.dbValue = myArray;
            viewModelPrefs.uiValue = myString;
            viewModelPrefs.uiValues = myArray;
    ...
    updatePrefilter3( data ) {
        ...
        let isPresentNow = viewModelPrefs.dbValue.includes( anySource );
        ...
        if( isValueAnySourceChanged ) {
            if( isPresentNow ) {
                ...
                viewModelPrefs.dbValue = dataSourceListWithAny;
                viewModelPrefs.uiValue = dataSourceListWithAny.toString();
                viewModelPrefs.uiValues = dataSourceListWithAny;
            ...
            // Checks all other options are selected or not.
            _.forEach( this._dataSourcesReceivedFromServer, function( val ) {
                if( !viewModelPrefs.dbValue.includes( val ) ) {
                    allSelected = false;
                }
            } );

            if( !allSelected ) {
                let newValues = JSON.parse( JSON.stringify( viewModelPrefs.dbValue ) );
                ...
                viewModelPrefs.dbValue = newValues;
