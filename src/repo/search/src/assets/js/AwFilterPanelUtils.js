...
export let accumulatePositivePendingAndActiveFilterMaps = ( activeFilterMap, positivePendingFilterMap ) => {
    let cumulativeFilterMap = positivePendingFilterMap && Object.keys( positivePendingFilterMap ).length > 0 ?
        _.cloneDeep( positivePendingFilterMap ) : {};
    let currentActiveFilterMap = activeFilterMap && Object.keys( activeFilterMap ).length > 0 ?
        _.cloneDeep( activeFilterMap ) : {};
    let currentPositivePendingFilterMap = positivePendingFilterMap && Object.keys( positivePendingFilterMap ).length > 0 ?
        _.cloneDeep( positivePendingFilterMap ) : {};
    for( let[ activeMapKey, activeMapValue ] of Object.entries( currentActiveFilterMap ) ) {
        let keyFound = false;
        for( let[ pendingMapKey, positiveMapValue ] of Object.entries( currentPositivePendingFilterMap ) ) {
...
export let getSearchFilterMapForFacetSearch = ( searchState, category ) => {
    ...
    let activeFilterMapInSearchState = searchStateContext.autoApplyFilters ?
        searchStateContext.activeFilterMap
        : AwFilterPanelUtils.accumulatePositivePendingAndActiveFilterMaps( searchStateContext.activeFilterMap, searchStateContext.positivePendingFilterMap );
...
export let updateCategoriesAfterFacetSearch = ( response, searchState, categoryForFacetSearchInput, category ) => {
    ...
    searchStateContext.categoriesExpandCollapseMap =
        setCategoryExpandCollapseStateInSearchState( searchStateContext.categoriesExpandCollapseMap, category.internalName, category.expand );
    searchStateContext = searchStateContext && searchStateContext.provider === fullTextSearchProviderName
        ? updateListOfExpandedCategoriesInSearchState( searchStateContext, categoryForFacetSearchInput.name, category.expand ) : searchStateContext;
...
export let updateExpandFlagForCategory = ( searchState, categoryForFacetSearchInput ) => {
    ...
    searchStateContext.categoriesExpandCollapseMap =
        setCategoryExpandCollapseStateInSearchState( searchStateContext.categoriesExpandCollapseMap, categoryForFacetSearchInput.name, currentCategoryExpandState );
    searchStateContext.categories = categories;
    searchStateContext = searchStateContext && searchStateContext.provider === fullTextSearchProviderName
        ? updateListOfExpandedCategoriesInSearchState( searchStateContext, categoryForFacetSearchInput.name, currentCategoryExpandState ) : searchStateContext;
    searchState.update( searchStateContext );
};
...
export let getToggleCommandSelection = ( prefValue, searchState ) => {
    ...
    searchHighlightingService.toggleColorFiltering();
...
const AwFilterPanelUtils = {
    ...
    accumulatePositivePendingAndActiveFilterMaps,
    showAllCategories,
    excludeCategory,
